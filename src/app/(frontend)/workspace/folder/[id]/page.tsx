	...
	return (
		<div>
			...
			{currentItems.length > 0 ? (
				<div
					className={` pt-3 w-full ${items.length > 0 ? "flex " : "flex items-center "}`}
				>
					{currentItems.map((item) => (
						<div key={item.id} className="grid grid-cols-3 sm:grid-cols-2 gap-3 md:grid-cols-3 lg:grid-cols-4 xl:grid-cols-8 2xl:grid-cols-8">
							{item.type === "folder" ? (
								<FolderIcon
									onClick={() => {
